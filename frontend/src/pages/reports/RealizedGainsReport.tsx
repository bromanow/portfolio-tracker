        ...
        : (
          <div className="space-y-6">
            <div className={`rounded-lg px-5 py-3 ${totalAll >= 0 ? 'bg-emerald-50 border border-emerald-200' : 'bg-red-50 border border-red-200'}`}>
              <span className="text-sm font-medium text-foreground">{rows.length} taxable dispositions · Total net gain: </span>
              <span className={`font-bold text-base ${totalAll >= 0 ? 'text-emerald-700' : 'text-red-600 dark:text-red-400'}`}>{fmtCAD(totalAll)}</span>
              <span className="text-xs text-muted-foreground ml-2">(50% taxable inclusion = {fmtCAD(totalAll * 0.5)})</span>
            </div>

            {byYear.length > 0 && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-card rounded-xl border border-border p-4">
                  <h3 className="font-semibold text-foreground mb-3 text-sm">Net Gain/Loss by Year</h3>
                ...
                <div className="bg-card rounded-xl border border-border p-4">
                  <h3 className="font-semibold text-foreground mb-3 text-sm">Net Gain/Loss by Security (top 12)</h3>
                  <ResponsiveContainer width="100%" height={220}>
            ...
            <div className="flex flex-wrap gap-3 items-center">
              ...
              {(['all', 'gains', 'losses'] as const).map(f => (
                <button key={f} onClick={() => setGainFilter(f)}
                  className={`px-3 py-1.5 text-sm rounded border capitalize ${gainFilter === f ? 'bg-primary text-white border-primary' : 'border-border text-muted-foreground hover:bg-muted/50'}`}>
                  {f}
                </button>
              ))}
                ...
                <table className="min-w-full text-sm divide-y divide-border">
                    ...
                    {viewMode === 'security' ? (
                      <>
                        {securityGroups.length === 0 && (
                          <tr><td colSpan={COLS} className="px-4 py-8 text-center text-muted-foreground">No realized gains/losses found.</td></tr>
                        )}
                            ...
                            {secRows.map((g, i) => {
                              ...
                              return (
                                <tr key={i} className="hover:bg-muted/50">
                                  <td className="px-4 py-2 text-xs text-muted-foreground whitespace-nowrap">{g.brokerage_name}</td>
                                  <td className="px-4 py-2 text-xs text-muted-foreground whitespace-nowrap max-w-[10rem] truncate">{g.account_name}</td>
                                  <td className="px-4 py-2 text-muted-foreground text-xs whitespace-nowrap">{g.date}</td>
                                  <td className="px-4 py-2 font-mono font-semibold"><TickerLink securityId={g.security_id} ticker={g.ticker} className="text-primary" /></td>
                                  <td className="px-4 py-2 text-right text-muted-foreground">{parseFloat(g.quantity).toLocaleString('en-CA', { maximumFractionDigits: 4 })}</td>
                                  <td className="px-4 py-2 text-right">{fmtCAD(g.proceeds_cad)}</td>
                                  <td className="px-4 py-2 text-right text-muted-foreground">{fmtCAD(g.acb_cad)}</td>
                                  <td className={`px-4 py-2 text-right font-semibold ${gain >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-500 dark:text-red-400'}`}>{fmtCAD(gain)}</td>
                                </tr>
                              )
                            })}
                            <tr className="bg-primary/10 border-t border-primary/20 text-xs font-semibold">
                              <td className="px-4 py-1.5 text-primary" colSpan={3}>{secRows.length} dispositions</td>
                              <td className="px-4 py-1.5 text-primary font-mono">{ticker} subtotal</td>
                              <td className="px-4 py-1.5" />
                              <td className="px-4 py-1.5 text-right text-primary">{fmtCAD(proceeds)}</td>
                              <td className="px-4 py-1.5 text-right text-primary">{fmtCAD(acb)}</td>
                              <td className={`px-4 py-1.5 text-right ${net >= 0 ? 'text-emerald-700' : 'text-red-600 dark:text-red-400'}`}>{fmtCAD(net)}</td>
                            </tr>
                          </Fragment>
                        ))}
                      </>
                        ...
                        {groupedRows.map(({ acctName, brokerage, rows: acctRows, proceeds, acb, net }) => (
                          <Fragment key={acctName}>
                            {acctRows.map((g, i) => {
                              const gain = parseFloat(g.gain_cad)
                              return (
                                <tr key={`${acctName}-${i}`} className="hover:bg-muted/50">
                                  <td className="px-4 py-2 text-xs text-muted-foreground whitespace-nowrap">{g.brokerage_name}</td>
                                  <td className="px-4 py-2 text-xs text-muted-foreground whitespace-nowrap max-w-[10rem] truncate" title={acctName}>{acctName}</td>
                                  <td className="px-4 py-2 text-muted-foreground text-xs whitespace-nowrap">{g.date}</td>
                                  <td className="px-4 py-2 font-mono font-semibold"><TickerLink securityId={g.security_id} ticker={g.ticker} className="text-primary" /></td>
                                  <td className="px-4 py-2 text-right text-muted-foreground">{parseFloat(g.quantity).toLocaleString('en-CA', { maximumFractionDigits: 4 })}</td>
                                  <td className="px-4 py-2 text-right">{fmtCAD(g.proceeds_cad)}</td>
                            ...
                            <tr className="bg-primary/10 border-t border-primary/20 text-xs font-semibold">
                              <td className="px-4 py-1.5 text-primary whitespace-nowrap">{brokerage}</td>
                              <td className="px-4 py-1.5 text-primary whitespace-nowrap max-w-[10rem] truncate" title={acctName}>{acctName} subtotal</td>
                              <td className="px-4 py-1.5 text-primary text-right" colSpan={3}>{acctRows.length} dispositions</td>
                              <td className="px-4 py-1.5 text-right text-primary">{fmtCAD(proceeds)}</td>
                              <td className="px-4 py-1.5 text-right text-primary">{fmtCAD(acb)}</td>
                              <td className={`px-4 py-1.5 text-right ${net >= 0 ? 'text-emerald-700' : 'text-red-600 dark:text-red-400'}`}>{fmtCAD(net)}</td>
                            </tr>
                  ...
                  {filtered.length > 0 && (
                    <tfoot className="bg-muted/50 border-t-2 border-border font-semibold text-sm">
                      <tr>
                        <td className="px-4 py-2.5 text-muted-foreground" colSpan={4}>{filtered.length} dispositions total</td>
                        <td className="px-4 py-2.5 text-right text-muted-foreground">—</td>
                        <td className="px-4 py-2.5 text-right">{fmtCAD(totalProceeds)}</td>
                        <td className="px-4 py-2.5 text-right text-muted-foreground">{fmtCAD(totalAcb)}</td>
                        <td className={`px-4 py-2.5 text-right ${totalFiltered >= 0 ? 'text-emerald-700' : 'text-red-600 dark:text-red-400'}`}>{fmtCAD(totalFiltered)}</td>
                      </tr>
                    </tfoot>
                  )}
                </table>
