...
export default function OptionRetypeTab() {
  ...
  return (
    <div className="space-y-4 max-w-3xl">
      <div>
        <h2 className="text-lg font-semibold text-foreground">Fix Option Types</h2>
        <p className="text-sm text-muted-foreground mt-1">
          Many option trades were imported as plain stock <code className="text-xs bg-accent px-1 rounded">SELL</code> /
          <code className="text-xs bg-accent px-1 rounded">BUY</code> instead of
          <code className="text-xs bg-accent px-1 rounded">OPTION_SELL</code> /
          <code className="text-xs bg-accent px-1 rounded">OPTION_BUY</code>. The ACB engine only applies option
      ...
      {isLoading ? (
        ...
        <div className="bg-card border border-border rounded-xl px-4 py-3 flex items-center gap-3">
          <CheckCircle className="h-5 w-5 text-emerald-500" /> <span className="text-sm text-muted-foreground">No mis-typed option transactions. All options use OPTION_* types.</span>
        </div>
      ) : (
        <>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            <div className="bg-muted/50 rounded-lg p-3"><div className="text-xs text-muted-foreground">SELL → OPTION_SELL</div><div className="text-2xl font-semibold text-foreground">{data.sell}</div></div>
            <div className="bg-muted/50 rounded-lg p-3"><div className="text-xs text-muted-foreground">BUY → OPTION_BUY</div><div className="text-2xl font-semibold text-foreground">{data.buy}</div></div>
            <div className="bg-muted/50 rounded-lg p-3 col-span-2 sm:col-span-1"><div className="text-xs text-muted-foreground">Total to retype</div><div className="text-2xl font-semibold text-primary">{data.total}</div></div>
          </div>
          ...
          {data.other > 0 && (
            <p className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
              Note: {data.other} option transactions are typed <code className="bg-card px-1 rounded">OTHER</code> — not touched by this retype; review separately.
            </p>
          )}

          <div className="bg-card border border-border rounded-xl overflow-hidden">
            <div className="px-4 py-2 bg-muted/50 text-xs uppercase tracking-wide text-muted-foreground font-medium">Sample (most recent 12)</div>
                ...
                <tbody>
                  {data.sample.map((s, i) => (
                    <tr key={i} className="border-t border-border">
                      ...
                      <td className="px-3 py-1.5"><span className="text-xs font-medium text-foreground">{s.type}</span> → <span className="text-xs font-medium text-primary">OPTION_{s.type}</span></td>
