...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-card border-t border-border flex items-stretch safe-area-inset-bottom">
      ...
      {isAdmin && (
        ...
        >
          {({ isActive }) => (
            <>
              <Settings className={`h-5 w-5 ${isActive ? 'text-primary' : 'text-muted-foreground'}`} />
              <span>Admin</span>
            </>
          )}
        </NavLink>
