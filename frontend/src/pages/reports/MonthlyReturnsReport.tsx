...
function pctCellCls(v: number | null | undefined): string {
  if (v === null || v === undefined) return 'text-muted-foreground/50'
  ...
  if (v >= -5) return 'text-red-600 dark:text-red-400 font-semibold'
  return 'text-red-700 font-bold'
}

function pctCellBg(v: number | null | undefined): string {
  if (v === null || v === undefined) return 'bg-muted/50'
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-4 items-end">
        <div>
          ...
          <select className="bg-background text-foreground border border-border rounded-lg px-3 py-2 text-sm" value={yearFrom} onChange={e => setYearFrom(Number(e.target.value))}>
            {yearOptions.map(y => <option key={y} value={y}>{y}</option>)}
          </select>
        </div>
        <div>
          <label className="block text-xs text-muted-foreground mb-1">To year</label>
          <select className="bg-background text-foreground border border-border rounded-lg px-3 py-2 text-sm" value={yearTo} onChange={e => setYearTo(Number(e.target.value))}>
            {yearOptions.map(y => <option key={y} value={y}>{y}</option>)}
          </select>
        </div>
        <div>
          ...
          <div className="flex gap-1">
            {(['none', 'brokerage', 'account_type'] as const).map(g => (
              <button key={g} onClick={() => setGroupBy(g)}
                className={`px-2.5 py-1.5 text-xs rounded font-medium transition-colors ${groupBy === g ? 'bg-primary text-white' : 'bg-accent text-muted-foreground hover:bg-accent'}`}>
        ...
        {groupBy !== 'none' && (
          <div className="flex gap-1 self-end">
            <button onClick={() => setCollapsed(new Set())} className="px-2 py-1.5 text-xs rounded border border-border hover:bg-muted/50 text-muted-foreground">Expand all</button>
            <button onClick={() => setCollapsed(new Set(groups.map(g => g.key)))} className="px-2 py-1.5 text-xs rounded border border-border hover:bg-muted/50 text-muted-foreground">Collapse all</button>
          </div>
        )}
        ...
        <div className="ml-auto flex flex-col items-end gap-1">
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors
              ${viewRefreshStatus === 'running' ? 'bg-accent text-muted-foreground border-border cursor-not-allowed'
              : viewRefreshStatus === 'done'    ? 'bg-green-50 text-green-600 dark:text-green-400 border-green-300'
      ...
      {/* Legend */}
      <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground bg-muted/50 rounded-lg px-4 py-2 border border-border">
        <span className="font-medium text-muted-foreground">Return scale:</span>
        {[['≥+5%', 'bg-emerald-100 text-emerald-800'], ['+2–5%', 'bg-emerald-50 text-emerald-700'], ['0–2%', 'text-emerald-600 dark:text-emerald-400'], ['0 to −2%', 'bg-red-50 text-red-500 dark:text-red-400'], ['−2 to −5%', 'bg-red-100 text-red-600 dark:text-red-400'], ['≤−5%', 'bg-red-200 text-red-700']].map(([label, cls]) => (
          <span key={label} className={`px-1.5 py-0.5 rounded text-xs font-medium ${cls}`}>{label}</span>
        ))}
        <span className="ml-auto text-muted-foreground">— = return suppressed (large inflow or no data)</span>
      </div>
        ...
        : (
          <div className="bg-card border border-border rounded-xl overflow-hidden">
            <div className="overflow-x-auto">
              <table className="text-xs border-collapse">
                <thead className="bg-muted/50">
                  {/* Year spanning row */}
                  <tr>
                    <th className="sticky left-0 z-30 bg-muted/50 px-3 py-2 text-left font-semibold text-muted-foreground whitespace-nowrap border-r border-border border-b border-border min-w-[168px]" rowSpan={2}>
                      Account
                    </th>
                    <th className="sticky left-[168px] z-30 bg-muted/50 px-2 py-2 text-left font-semibold text-muted-foreground whitespace-nowrap border-r border-border border-b border-border min-w-[88px]" rowSpan={2}>
                      Brokerage
                    </th>
                    <th className="sticky left-[256px] z-30 bg-muted/50 px-2 py-2 text-left font-semibold text-muted-foreground whitespace-nowrap border-r-2 border-border border-b border-border min-w-[60px]" rowSpan={2}>
                      Type
                    </th>
                    {years.map(y => (
                      <th key={y} colSpan={(yearToMonths[y]?.length ?? 0) + 1}
                        className="px-2 py-1.5 text-center font-bold text-foreground border-l-2 border-border border-b border-border bg-muted/50">
                        {y}
                      </th>
                    ))}
                  </tr>
                    ...
                    {years.map(y => (
                      <Fragment key={y}>
                        {(yearToMonths[y] ?? []).map(mk => (
                          <th key={mk} className="px-0.5 py-1.5 text-center text-muted-foreground font-medium border-l border-border min-w-[42px] whitespace-nowrap">
                            {shortMonth(mk)}
                          </th>
                        ))}
                        <th className="px-1 py-1.5 text-center text-foreground font-bold border-l-2 border-border min-w-[52px] bg-accent whitespace-nowrap">
                          Ann.
                        </th>
                      </Fragment>
                    ))}
                ...
                <tbody className="divide-y divide-border">
                  {groups.length === 0 && (
                    <tr><td colSpan={totalCols} className="px-4 py-8 text-center text-muted-foreground">No data found for selected filters.</td></tr>
                  )}
                  {groups.map(group => (
                    <Fragment key={group.key}>
                      {/* Group header */}
                      {groupBy !== 'none' && group.label && (
                        <tr className="bg-accent hover:bg-accent cursor-pointer select-none" onClick={() => toggleGroup(group.key)}>
                          <td className="sticky left-0 z-10 bg-accent px-3 py-1.5 border-r border-border">
                            <div className="flex items-center gap-1.5">
                              {collapsed.has(group.key)
                                ? <ChevronRight className="h-3.5 w-3.5 text-muted-foreground flex-shrink-0" />
                                : <ChevronDown  className="h-3.5 w-3.5 text-muted-foreground flex-shrink-0" />}
                              <span className="font-semibold text-foreground whitespace-nowrap">{group.label}</span>
                              <span className="text-muted-foreground font-normal">{group.rows.length} acct{group.rows.length !== 1 ? 's' : ''}</span>
                            </div>
                          </td>
                          <td className="sticky left-[168px] z-10 bg-accent border-r border-border" />
                          <td className="sticky left-[256px] z-10 bg-accent border-r-2 border-border" />
                          {years.map(y => (
                            <Fragment key={y}>
                              {(yearToMonths[y] ?? []).map(mk => <td key={mk} className="border-l border-border" />)}
                              <td className="border-l-2 border-border bg-accent" />
                            </Fragment>
                          ))}
                        </tr>
                      )}
                      ...
                      {!collapsed.has(group.key) && group.rows.map(row => (
                        <tr key={row.account_ids.join('-')} className="hover:bg-primary/30 group/row">
                          <td className="sticky left-0 z-10 bg-card group-hover/row:bg-primary/40 px-3 py-1.5 font-medium text-foreground whitespace-nowrap truncate border-r border-border max-w-[168px]" title={row.account_name}>
                            {row.account_name}
                          </td>
                          <td className="sticky left-[168px] z-10 bg-card group-hover/row:bg-primary/40 px-2 py-1.5 text-muted-foreground whitespace-nowrap border-r border-border max-w-[88px] truncate" title={row.brokerage}>
                            {row.brokerage}
                          </td>
                          <td className="sticky left-[256px] z-10 bg-card group-hover/row:bg-primary/40 px-2 py-1.5 whitespace-nowrap border-r-2 border-border">
                            <span className="px-1.5 py-0.5 rounded bg-accent text-muted-foreground">{row.account_type}</span>
                          </td>
                          {years.map(y => (
                            <Fragment key={y}>
                              {(yearToMonths[y] ?? []).map(mk => {
                                ...
                                return (
                                  <td key={mk}
                                    className={`px-0.5 py-1.5 text-center border-l border-border tabular-nums ${pctCellBg(v)} ${pctCellCls(v)}`}>
                                    {fmtPct(v)}
                                  </td>
                                )
                              })}
                              {(() => {
                                ...
                                return (
                                  <td className={`px-1 py-1.5 text-center border-l-2 border-border tabular-nums ${pctCellBg(v)} ${pctCellCls(v)} bg-opacity-70`}>
                                    {fmtPct(v)}
                                  </td>
                                )
                              })()}
            ...
            {filteredRows.length > 0 && (
              <div className="px-4 py-2 border-t border-border text-xs text-muted-foreground bg-muted/50">
